...
export default function UnderwriteButton({pawnShopContract, jsonRPCContract, ticketInfo, account, allowance, interestRate, loanAmount, duration, loanUpdatedCallback}){
    ...
    const get110Percent = (value: ethers.BigNumber) => {
        return value.add(value.mul(10).div(100))
    }

    const get90Percent = (value: ethers.BigNumber) => {
        return value.add(value.mul(90).div(100))
    }
    ...
    const isDisabled = () => {
        return loanAmount.gt(allowance) || (!checkHas10PercentImprovement() && isFilled())
    }
