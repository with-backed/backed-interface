    ...
    return(
        <div>
            ...
            <div className='collateralDetails'>
                ...
                <p><a target="_blank" href={process.env.NEXT_PUBLIC_OPENSEA_URL + "/assets/" +  nftInfo.address + '/' + nftInfo.id.toString()}> View on OpenSea</a></p>
                <p><a target="_blank" href={process.env.NEXT_PUBLIC_ETHERSCAN_URL + "/token/" +  nftInfo.address + '?a=' + nftInfo.id.toString()}> View on Etherscan</a></p>
                <p><b>Contract Address</b> {nftInfo.address.slice(0, 7)} ... {nftInfo.address.slice(35, -1)} </p>
                <p><b>ID</b> {nftInfo.id.toString()} </p>
            </div>
        </div>
    )
}