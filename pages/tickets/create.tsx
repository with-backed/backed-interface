...
export default function Create({}){
    ...
    const [collateralTokenID, setCollateralTokenID] = useState(ethers.BigNumber.from(0))
    const [isValidCollateral, setIsValidCollateral] = useState(false)

    return(
        <div id='ticket-page-wrapper'> 
            <PawnShopHeader account={account} setAccount={setAccount}  message={'create a loan'} />
        
        <fieldset className='standard-fieldset float-left'> 
            <legend> pawn your NFT </legend>
            {account == null ? 
            <ConnectWallet account={account} addressSetCallback={setAccount}  buttonType={1} />
            :  <CreateTicketForm 
                    account={account}
        ...
        { !isValidCollateral ? '' : <div className='float-left'><CollateralMediaCard  collateralAddress={collateralAddress} collateralID={collateralTokenID} /></div> }
